...
public class ScreenShotUtil {
	...
	private static final String SCREENSHOT_FILE_NAME_SUFFIX = ".png";
	/** 获取屏幕密度 */
     ...
     */
    public static Bitmap saveScreenShot(final String liverName, Activity activity, final File dirName) {
      final Bitmap takeShot = takeShot(activity);
        //新起一个线程进行保存
       ...
       new Thread(new Runnable() {
			...
			public void run() {
				...
				savePic(liverName,takeShot, dirName);
			}
		}).start();    
       return takeShot;
    }
    ...
    private static Bitmap takeShot(Activity activity) {
    	...
    	if(null != activity){
            ...
            b = Bitmap.createBitmap(view.getDrawingCache(), 0, statusBarHeight, width, height - statusBarHeight);
            // 销毁缓存信息
    ...
    // 保存到sdcard
    private static void savePic(String liverName ,Bitmap b, File dirName) {

    	File screenFileName =  new File(dirName ,
                DateFormatUtil.getFileName() + SCREENSHOT_FILE_NAME_SUFFIX);
        FileOutputStream fos = null;
        try {
            fos = new FileOutputStream(screenFileName);
            if (null != fos) {

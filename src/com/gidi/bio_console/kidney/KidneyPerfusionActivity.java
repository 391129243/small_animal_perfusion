...
import com.gidi.bio_console.kidney.KidneyPerfusionFragment.OnKidneyPerfusionListener;
import com.gidi.bio_console.kidney.KidneySettingFragment.onKidneySettingListener;
...
public class KidneyPerfusionActivity extends BaseFragmentActivity implements OnClickListener, 
onKidneySettingListener, OnNetWorkChangedListener, 
OnKidneyPerfusionListener {

	private static final String TAG = "PerfusionActivity";
	private Context mContext;
	private BioConsoleApplication mApplication;
	private KidneyPerfusionActivity mActivity;
	...
	private ControllerKidneyFragment mFragmentController;
	private FragmentManager mFragmentManager;
	private Fragment mCurrentFrag;
	private KidneyPerfusionFragment mPerfusionFragment;	
	private KidneySettingFragment mSettingFragment;
	...
	/**肝动脉灌注模式 0是恒压,1是搏动**/
	private int mArtMode = Constants.LEFT_ARTERY_CONST_PRESSURE_MODE;
	/**门静脉灌注模式 2是恒压; 3是恒流**/
	...
	private static final int MSG_UPDATE_VEIN_PRESSURE = 1001;
	...
	protected int getLayoutId() {
		// TODO Auto-generated method stub
		return R.layout.module_activity_kidney_main_perfusion;
	}
	...
	private void initVariable(){
		...
		mFragmentController = ControllerKidneyFragment.getInstance(KidneyPerfusionActivity.this, R.id.perfusion_container_fragment);
		...
		setTabSelection(3);
		mPerfusionFragment = (KidneyPerfusionFragment)mFragmentController.getFragment(3);
		mPerfusionFragment.setOnPerfusionListener(this);
		mSettingFragment = (KidneySettingFragment)mFragmentController.getFragment(2);
		mSettingFragment.setKidneySettingListener(this);
		...
		mTitleImg.setImageResource(R.drawable.icon_kidney);
...
//	}
	...
	private void bindService() {
		// TODO Auto-generated method stub
		Intent service = new Intent(KidneyPerfusionActivity.this, BackService.class);
...
/*		case R.id.main_perfusion_add_bloodgas_record_img:
	...
	private void screenShot(){
        ...
        String subFileDir = kidneyNum + "_" + DateFormatUtil.getNowDate();
        File liverfileDir = new File(PATH_SCREENSHOT, subFileDir);
        if(!liverfileDir.exists() || !liverfileDir.isDirectory()){
        	liverfileDir.mkdir();
        }
        
        Bitmap bitmap = ScreenShotUtil.saveScreenShot(kidneyNum,(KidneyPerfusionActivity)mContext, liverfileDir);
	...
	private void sendArtPerfusionMsg(){
		if(mArtMode == Constants.LEFT_ARTERY_CONST_PRESSURE_MODE){
			...
			String strPulseArtmsg = StringUtil.appendMinMaxPre(SerialPortInstruct.SET_TARGET_PULSE_PRESSURE_ONE,
					(int)mArtMinTarPre,(int)mArtMaxTarPre);
			...
			//左肾动脉恒流
			float mArtTarFlow = PreferenceUtil.getInstance(getApplicationContext())
					.getFloatValue(SharedConstants.TARGET_LEFT_KIDNED_CONST_FLOW, 0.0f);
			String strArtConstFMsg = StringUtil.appendMsg(SerialPortInstruct.SET_TARGET_FLOW_ONE, (int)(mArtTarFlow),"0");
			PreferenceUtil.getInstance(getApplicationContext()).setValueByName(SharedConstants.LASTEST_ARTERY_TARGET_ORDER, strArtConstFMsg);
			if(null != mBinder){
				mBinder.sendSerialCmdMsg(strArtConstFMsg);
			}
		}		
		sendLimitSpeedMsg();
	}
	...
	private void sendVeinPerfusionMsg(){
		...
		if(mVeinMode == Constants.RIGHT_ARTERY_CONST_PRESSURE_MODE){
			...
			//右肾动脉搏动
			float mRArtMinTarPre = PreferenceUtil.getInstance(getApplicationContext())
					.getFloatValue(SharedConstants.TARGET_RIGHT_KIDNED_PRESSURE_MIN, 0.0f);
			float mRArtMaxTarPre = PreferenceUtil.getInstance(getApplicationContext())
					.getFloatValue(SharedConstants.TARGET_RIGHT_KIDNED_PRESSURE_MAX, 0.0f);
			strVeinTarMsg = StringUtil.appendMinMaxPre(SerialPortInstruct.SET_TARGET_PULSE_PRESSURE_TWO,
					(int)mRArtMinTarPre,(int)mRArtMaxTarPre);			
			if(null != mBinder){
				mBinder.sendSerialCmdMsg(strVeinTarMsg);
...
/*	*//**插入一条血气数据至血气表中**//*
	...
	private void checkBatteryState(int battery_level){
		if(!isBatteryAbnormal){
			...
			if(isUseBattery){
				if(battery_level <= 30){
					if(mSystemAlarmMgr != null && mUIHandler != null){
				...
				if(null != mSystemAlarmMgr && mUIHandler != null){
					if(mSystemAlarmMgr.isBatteryAlarm()){
						mSystemAlarmMgr.setBatteryAlarm(false);
						mUIHandler.sendEmptyMessage(Constants.MSG_NORMAL_INFO);					
					}	
	...
	public static class UIHandler extends Handler{
		...
		WeakReference<KidneyPerfusionActivity> mActivityReference;
		
		public UIHandler(KidneyPerfusionActivity mActivity){
			mActivityReference = new WeakReference<KidneyPerfusionActivity>(mActivity);
		}

		@Override
		public void handleMessage(Message msg) {
			super.handleMessage(msg);
			final KidneyPerfusionActivity mActivity = mActivityReference.get();
			if(null == mActivity){
				return;
			}
			String detaillMsg= "";
				...
				if(pump1_Qreal != null && null != mApplication
					...
					if(!pump1_Qreal.equals(DefValues.DEDAULT_NULL)&& !mApplication.isStopPump()){	
						...
						if(pump1_Qreal.equals("255.0")){	
							displayToast(R.string.error_left_kidney_artery_line);
						}
					}else{
					...
					if(!mApplication.isStopPump()){
						if(!pump2_Qreal.equals(DefValues.DEDAULT_NULL)){	
							...
							if(pump2_Qreal.equals("255.0")){
								displayToast(R.string.error_right_kidney_artery_line);						
							}
						}else{
	...
	private static class SQLiteAsyncTask extends AsyncTask<BloodGasSamplingBean, Integer, Long>{
		...
		private WeakReference<KidneyPerfusionActivity> weakAty;
		
        public SQLiteAsyncTask(KidneyPerfusionActivity activity,BloodGasSamplingBean bloodGasBean){
            weakAty = new WeakReference<KidneyPerfusionActivity>(activity);
		...
		protected Long doInBackground(BloodGasSamplingBean... params) {
			...
			KidneyPerfusionActivity mActivity = weakAty.get();
			if(null != mActivity){
	...
	private void onEndPerfusion() {
		...
		if(null != mApplication){
			...
			mApplication.restoreKidneyVariable();
		}
		
		Intent endIntent = new Intent(KidneyPerfusionActivity.this, PowerOnActivity.class);
	 ...
	 * 门静脉模式：mVeinMode = 3：搏动；mVeinMode = 4：恒压 5：恒流
	 *        
	 */
	@Override
	public void onFinishSetPerfison(boolean isArtSet, boolean isVeinSet,boolean isArtModeChange, boolean isVeinModeChange) {
		Log.i("111", "---onFinishSetPerfison---" + "isArtSet---" + "   " + isArtSet + "isVeinSet" + "   "+ isVeinSet);
		Log.i("111", "---onFinishSetPerfison---" + "isArtModeChange---" + "   " + isArtModeChange + "isVeinModeChange" + "   "+ isVeinModeChange);
		// TODO Auto-generated method stub
		if(mApplication.isStopPump()){
			if(isArtModeChange){
				mArtMode = PreferenceUtil.getInstance(getApplicationContext())

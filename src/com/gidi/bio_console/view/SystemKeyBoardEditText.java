...
import com.gidi.bio_console.listener.FormatTextWatcher;
import com.gidi.bio_console.listener.KeyBoardActionListener;
import com.gidi.bio_console.listener.OnEditFocusChangeListener;
import com.gidi.bio_console.listener.SystemOnKeyboardActionListener;
...
public class SystemKeyBoardEditText extends KeyBoardEditText {
    ...
    private OnKeyBoardOutsideDismissListener mOutSideDismissListener;
                ...
                if (!isShowing()) {
                    if (event.getAction() == MotionEvent.ACTION_DOWN) {
                        if (focusEnable) {
                            ...
                            if (enable) {
                            	CommonUtil.disableShowSoftInput(SystemKeyBoardEditText.this);
                                showKeyboardWindow(currentView);
                            }
                        } else {
            ...
            public void onFocusChange(View v, final boolean hasFocus) {
                ...
                if (STATUE == READY) {
                    mHandler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            if (STATUE == STAR) {                           
                                if (!hasFocus && outSideEnable) {
                                	if(focusMark == 1 || focusMark == 3){
                                        dismissKeyboardWindow();
                                        if(null != mOutSideDismissListener){
                                        	mOutSideDismissListener.OnKedboardOutDismiss();
                                        }
                                	} 
                                }
                               STATUE = READY;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        initPopWindow(mSystemKeyboard,this.getX(),this.getY());
    ...
    public interface OnKeyBoardOutsideDismissListener {

    	void OnKedboardOutDismiss();
    }
    
    public void setOutSideDissmissListener(OnKeyBoardOutsideDismissListener dismissListener){
    	this.mOutSideDismissListener = dismissListener;
    }
    /**
     * 设置键盘背景 可以用此背景设置线条粗细
     *

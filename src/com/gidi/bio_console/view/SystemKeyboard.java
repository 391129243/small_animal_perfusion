...
import com.gidi.bio_console.bean.KeyModel;
import com.gidi.bio_console.listener.IKeyBoardUIChange;
...
public class SystemKeyboard extends FrameLayout {
    ...
    private void initKeyBoard(Context context, int xmlLayoutResId) {
        ...
        keyboardView = (MyKeyboardView) LayoutInflater.from(context).inflate(R.layout.layout_custom_keyboardview, null);
    ...
    private void randomKey() {
        ...
        for (int i = 0, size = keyList.size(); i < size; i++) {
            ...
            if (label != null && CommonUtil.isNumeric(label.toString())) {
                newKeyList.add(key);
            }
        }
    ...
    public void setEditText(@NonNull EditText editText, boolean isOpenNativeKeyBoard) {
        CommonUtil.checkNull(actionListener,"Please check if xmlLayoutResId is set");
        ...
        if (isOpenNativeKeyBoard) {
            ...
            CommonUtil.disableShowSoftInput(editText);
